...
renderPlotly({p3 <- df_summary %>% 
  ...
  arrange(desc(num_ratings)) %>% 
  head(5) %>%     
  ggplot(aes(x = reorder(restaurant_name, -num_ratings), y = avg_cost_of_the_order)) + 
           geom_bar(stat = "identity") +
           labs(x = "Restaurant Name",
...
renderPlotly({p4 <- df_summary %>% 
  ...
  arrange(desc(num_ratings)) %>% 
  head(5) %>% 
  ggplot(aes(x = reorder(restaurant_name, -num_ratings), y = avg_total_time)) + 
           geom_bar(stat = "identity") +
           labs(x = "Restaurant Name",
...
```{r}
radioButtons(inputId = "four_rating_input", 
             label = h2("Select Ratings"),
             choices = c("rated 4 or more", "rated less than 4"),
             inline = TRUE,
             selected = "rated 4 or more")
```
...
```{r}
renderPlotly({p5 <- df_summary %>% 
    filter(cuisine_type%in%input$cuisine_type_input) %>% 
        filter(num_reviews_5_plus == input$five_plus_input) %>% 
  filter(avg_cost_of_the_order>input$price_input[1], avg_cost_of_the_order<input$price_input[2]) %>% 
  filter(avg_total_time>input$time_input[1], avg_total_time<input$time_input[2]) %>% 
    filter(four_plus_rating==input$four_rating_input) %>% 
  arrange(desc(avg_cost_of_the_order)) %>% 
  head(5) %>% 
  ggplot(aes(x = reorder(restaurant_name, -avg_cost_of_the_order), y = avg_cost_of_the_order)) + 
           geom_bar(stat = "identity") +
           labs(x = "Restaurant Name",
                y = "Average Cost") +
           theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
Code Organization of the dashboard:

- helpful to switch between Source & Visual in R to identify where my pages, columns, and tabs start & end to keep everything where I want it

- helpful to have consistent labeling for each part of the final assignment (ex: "final_Q1.rmd")

Errors & Challenges: 

- keeping code organized

- switching between the two shiny accounts

- debugging code

- having my data set be stored on github and not locally

Advice:

- map out each page of the dashboard and what you want displayed on each page, column, tab
